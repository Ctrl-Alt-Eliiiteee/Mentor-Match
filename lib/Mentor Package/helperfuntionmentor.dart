...
class Somefuntionsmentor {
  static Future<List<List<String>>> studentssessionlistupcoming() async {
    ...
    final QuerySnapshot snap = await FirebaseFirestore.instance
        ...
        .where('CompValue', isGreaterThanOrEqualTo: value)
        .get();
  ...
  static Future<List<List<String>>> studentssessionlistprevious() async {
    List<List<String>> temp = [];
    DateTime date = DateTime.now();
    int value = (date.hour.toDouble() + (date.minute.toDouble() / 60)).toInt();
    final QuerySnapshot snap = await FirebaseFirestore.instance
        ...
        .doc(Email)
        .collection("UniqueDetails")
        .where('CompValue', isLessThan: value)
        .get();
    List<DocumentSnapshot> results = snap.docs;
  ...
  static Future<List<String>> currentsession() async {
    DateTime date = DateTime.now();
    int value = (date.hour.toDouble() + (date.minute.toDouble() / 60)).toInt();
    final QuerySnapshot snap = await FirebaseFirestore.instance
        .collection("SessionDetails")
